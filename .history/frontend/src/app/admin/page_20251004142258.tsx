...
const AdminPage = () => {
  console.log('Attempting to render AdminPage...');
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`http://localhost:3001/api/v1/oobe-pages/${id}`);
  ...
  const handleOk = async () => {
    try {
      ...
      if (editingPage) {
        await axios.put(`http://localhost:3001/api/v1/oobe-pages/${editingPage.id}`, values);
        message.success('Page updated successfully');
      } else {
        await axios.post('http://localhost:3001/api/v1/oobe-pages', values);
